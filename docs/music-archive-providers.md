...
‘확인하지 못함’은 비공개 제휴 API도 존재하지 않는다는 단정이 아니다. 소비자 웹사이트를 볼 수 있다는 사실만으로 대량 수집·상업 재표시·이미지 이용을 허가받은 것으로 취급하지 않는다. 2026-09-10 후속 변경은 회원이 명시한 멜론·지니 앨범 URL의 크레딧 재조회에 기존 파서와 `createReviewUrlFetcher`를 재사용한다. 아티스트 전체 대량수집은 추가하지 않으며 파서에서 읽은 가사는 아카이브에 보관하지 않는다.
...
[데이터 라이선스](https://musicbrainz.org/doc/About/Data_License)와 [공식 코어 데이터 CC0 고지](https://github.com/metabrainz/musicbrainz-server/blob/master/admin/COPYING-PublicDomain)를 구분한다. 코어 사실 메타데이터는 CC0이고 부가 데이터에는 별도 라이선스가 있다. MusicBrainz 어댑터는 태그·평점·주석·가사·음원·사진·커버를 요청/저장하지 않는다. 2026-09-10부터 작품의 작사·작곡·편곡 관계를 요청하고 역할별 저작자 정보를 저장한다. 이름·발매일·타입·크레딧·식별자·디스크/트랙 순서·길이 등 사용한 필드만 정규화한다. MusicBrainz 자체는 커버를 제공하지 않아 `imageUrl: null`을 반환한다. Apple API나 명시된 멜론·지니 앨범 페이지에서 확보한 공식 썸네일 URL은 별도로 표시한다. 외부 ID/URL과 MusicBrainz 출처·확인시각을 유지하고 사용자의 자체 업무 기록과 분리한다. 데이터 라이선스의 허용 범위와 API 서비스 계약의 추가 보관 조건을 운영자가 함께 확인해야 한다.
...
## 2026-09-10 선택 앨범 정보 재조회

이 변경에서는 Apple 공식 API의 썸네일 URL, 명시적으로 연결된 멜론·지니 앨범 페이지의 공개 크레딧·썸네일, MusicBrainz 작품 관계 및 소유권이 검증된 본인 심의 신청 크레딧을 사용한다. `refresh-metadata`는 저장한 앨범 ID/공식 URL에만 접근하고 이름으로 다른 서비스의 앨범을 추정하지 않는다. 멜론·지니는 기존의 호스트·경로·ID·리다이렉트·응답 크기·동시성 제한을 재사용하고 조회 전체에 45초 제한을 더한다. 가사와 이미지 파일은 아카이브에 저장하지 않는다. 사이트 응답 누락/접근 제한은 기존 회원 기록을 지우지 않는다. 제공처 상태의 `link_only`는 아티스트 전체 카탈로그 자동수집이 없다는 의미이며 선택 앨범 링크 재조회와 구분한다.

읽기 검증: 2026-09-10 기존 지니 샘플 앨범 ID `87816941`의 공개 페이지를 제한된 fetcher로 조회해 2곡 모두의 저작자 필드와 썸네일 URL을 확인했다(`partial:false`). 운영 아카이브에는 쓰지 않았다.

## 2026-09-12 가져오기 보완

새로 가져온 앨범은 해당 회원의 정확한 앨범 연결/심의 범위에 멜론·지니 URL이 있으면 저작자와 자켓을 함께 보완한다. 부가 조회는 한 수집 단계에서 15초로 제한되며, 사이트 오류가 원래 앨범·트랙 가져오기를 취소하지 않는다. 제목 검색만으로 다른 앨범의 크레딧을 연결하지 않는다. 기존 저작자 역할과 등록 기록에 연결된 저작물은 일부 응답 누락으로 삭제하지 않는다.

Apple 자켓 재조회는 아티스트 연결이나 미국 전체 트랙 조회 없이 저장된 정확한 앨범 ID/URL로 한국 카탈로그의 썸네일을 확인한다. 관리자가 직접 추가한 앨범에도 동일하게 적용되며 사용자가 수정한 자켓은 보존한다. MusicBrainz 저작자 재조회는 Apple 링크가 있어도 별도로 실행한다. 수동 재조회 전체 45초 제한을 두 카탈로그에도 적용하고, 아직 처리하지 않은 앨범은 다음 묶음에서 이어서 확인한다.

2026-09-12 읽기 전용 실제 API 확인: Apple 앨범 `1259083959`의 한국 앨범 조회에서 공식 `is1-ssl.mzstatic.com` 썸네일 URL을 수신했다. 이 API 확인에서는 운영 회원 데이터를 변경하지 않았다. 자동 연결·부분 응답 보존·수동 수정 보호·중복 방지는 모의 응답을 사용하는 테스트로 별도 검증한다. 이후 커밋 `244dd60`을 기존 Render 서비스에 운영 배포했으며, 결과는 [운영 반영 기록](review-docs/deployment-report.md)에 정리했다.
