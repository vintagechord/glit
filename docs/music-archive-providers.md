...
2026-09-12 읽기 전용 실제 API 확인: Apple 앨범 `1259083959`의 한국 앨범 조회에서 공식 `is1-ssl.mzstatic.com` 썸네일 URL을 수신했다. 운영 회원 데이터 변경이나 배포는 하지 않았다. 자동 연결·부분 응답 보존·수동 수정 보호·중복 방지는 모의 응답을 사용하는 테스트로 별도 검증한다.
