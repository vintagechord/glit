-- Serialize submission edits across application instances. A short DB-backed
-- lease keeps the parent non-payable while its dependent rows are replaced;
-- the final RPC commits tracks/files/reviews and the final state atomically.

alter table public.submissions
  add column if not exists save_lease_token uuid,
  add column if not exists save_lease_expires_at timestamptz;

alter table public.submissions
  drop constraint if exists submissions_save_lease_pair;
alter table public.submissions
  add constraint submissions_save_lease_pair check (
    (save_lease_token is null and save_lease_expires_at is null)
    or (save_lease_token is not null and save_lease_expires_at is not null)
  );

create index if not exists submissions_save_lease_expiry_idx
  on public.submissions (save_lease_expires_at)
  where save_lease_token is not null;
...
begin
  ...
  set status = 'DRAFT',
      ...
      save_lease_token = p_lease_token,
...
begin
  ...
  if v_submission.type = 'ALBUM' then
    if not coalesce(p_replace_tracks, false) then
      raise exception 'ALBUM_TRACK_REPLACEMENT_REQUIRED'
        using errcode = '22023';
    end if;
...
-- Defense in depth if browser update policies are broadened in the future.
create or replace function public.protect_submission_save_lease_fields()
returns trigger
language plpgsql
security definer
...
begin
  ...
  if new.save_lease_token is distinct from old.save_lease_token
    or new.save_lease_expires_at is distinct from old.save_lease_expires_at
  then
    raise exception 'Submission save lease fields require a trusted server.'
      using errcode = '42501';
  end if;
  return new;
end;
...
drop trigger if exists protect_submission_save_lease_fields
  on public.submissions;
create trigger protect_submission_save_lease_fields
before update of save_lease_token, save_lease_expires_at
on public.submissions
for each row execute function public.protect_submission_save_lease_fields();

revoke all on function public.protect_submission_save_lease_fields()
  from public, anon, authenticated;
