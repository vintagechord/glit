create table if not exists public.profanity_terms (
  id uuid primary key default gen_random_uuid(),
  term text not null,
  language text not null default 'KO',
  is_active boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create index if not exists profanity_terms_language_idx
  on public.profanity_terms (language);

create index if not exists profanity_terms_active_idx
  on public.profanity_terms (is_active);

alter table public.profanity_terms enable row level security;

do $$
begin
  create policy "Public read profanity terms"
    on public.profanity_terms
    for select
    using (is_active = true);
exception
  when duplicate_object then
    null;
end $$;
