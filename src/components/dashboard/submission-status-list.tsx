...
export function SubmissionStatusList({
  ...
  return (
    <>
                ...
                <div className="flex min-w-0 flex-wrap items-center gap-2 md:justify-end">
                  ...
                  {hasPaymentAction && (
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `${localePrefix}/mypage/cart?focus=${submission.id}`,
                        )
                      }
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="shrink-0 rounded-full border border-border/70 px-3 py-1 text-sm font-semibold uppercase tracking-[0.2em] text-foreground"
              >
                닫기
              </button>
            </div>
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-border/60 bg-background/80">
              <div className="grid grid-cols-[minmax(0,1fr)_minmax(100px,auto)] items-center gap-2 border-b border-border/60 bg-muted/40 px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground sm:grid-cols-[minmax(0,1.4fr)_minmax(110px,0.8fr)_96px]">
                <span>방송국</span>
                  ...
                  {[...activeSubmission.station_reviews]
                    ...
                    return (
                      <div
                        ...
                        className="grid grid-cols-[minmax(0,1fr)_minmax(100px,auto)] items-center gap-2 rounded-xl border border-border/50 bg-background/80 px-3 py-2 text-sm sm:grid-cols-[minmax(0,1.4fr)_minmax(110px,0.8fr)_96px]"
                      >
      ...
      {trackResultModal ? (
        <div
          className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 px-4 py-4"
          onClick={() => setTrackResultModal(null)}
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-label={`${trackResultModal.stationName} 트랙별 결과`}
            className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-2xl border border-border/60 bg-background p-5 shadow-xl sm:p-6"
            onClick={(event) => event.stopPropagation()}
          >
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
              트랙별 결과
