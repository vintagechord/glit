...
export function Button({ children, primary = false, className = "", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { primary?: boolean }) {
  return <button type="button" {...props} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border-2 px-3 py-2 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-foreground disabled:cursor-not-allowed disabled:opacity-50 ${primary ? "border-foreground bg-foreground text-background hover:opacity-85" : "border-border bg-card text-foreground hover:border-foreground"} ${className}`}>{children}</button>;
}
...
export function Notice({ children, error = false }: { children: ReactNode; error?: boolean }) {
  return <div role={error ? "alert" : "note"} className={`rounded-lg border p-3 text-sm leading-6 ${error ? "border-red-300 bg-red-50 text-red-900 dark:bg-red-950 dark:text-red-100" : "border-border bg-background text-foreground"}`}>{children}</div>;
}
