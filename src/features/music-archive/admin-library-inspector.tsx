...
export type AdminLibrarySummary = {
  ...
  track_count: number; archived_at: string | null; updated_at: string;
};
export type AdminLibraryIndex = { libraries?: AdminLibrarySummary[]; total?: number; nextPage?: number | null };
...
function LibraryOperationsDetail({ detail, onRetry, onResolve }: { detail: AdminLibraryDetail; onRetry: (id: string) => Promise<void>; onResolve: ResolveConflict }) {
  const { library } = detail;
  const [tab, setTab] = useState("sources");
  const tabs = [{ id: "sources", label: "출처·연결" }, { id: "tasks", label: "업무 기록" }, { id: "history", label: "변경 이력" }];
...
export function AdminLibraryInspector({ initial }: { initial: AdminLibraryIndex }) {
  ...
  useEffect(() => { if (!activeQuery && page === 0) setIndex(initial); }, [initial, activeQuery, page]);
  useEffect(() => () => { requestId.current++; detailRequestId.current++; }, []);

  async function search(nextPage: number, q = activeQuery) {
    const request = ++requestId.current;
    setLoading(true); setError("");
    try {
      const result = await archiveRequest<AdminLibraryIndex>(`?action=admin&q=${encodeURIComponent(q)}&page=${nextPage}`);
      if (requestId.current === request) { setIndex(result); setPage(nextPage); setActiveQuery(q); }
  ...
  return <section className={panelClass} aria-labelledby="admin-libraries-heading">
    ...
    <p className="mt-2 text-sm leading-6 text-muted-foreground">정보 출처·연결 상태·업무 기록·변경 이력은 이 관리자 화면에서 확인합니다.</p>
    <form className="mt-4 flex flex-wrap items-end gap-2" onSubmit={submit}><div className="min-w-0 flex-1"><Field label="아티스트 이름 검색"><input className={inputClass} value={query} maxLength={100} placeholder="아티스트 이름" onChange={event => setQuery(event.target.value)} /></Field></div><Button type="submit" disabled={loading}><Search className="h-4 w-4" aria-hidden />검색</Button></form>
    {error && <div className="mt-3"><Notice error>{error}</Notice></div>}
    {loading && <p role="status" className="mt-3 text-sm">아카이브 목록을 불러오고 있습니다.</p>}
    <div className="mt-4 space-y-3">{libraries.map(library => <article key={library.id} className="min-w-0 rounded-lg border border-border p-3 sm:p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><strong className="break-words">{library.artist_name}</strong>{library.archived_at && <span className="ml-2"><Badge>보관됨</Badge></span>}<p className="mt-1 text-sm text-muted-foreground">앨범 {library.release_count}개 · 트랙 {library.track_count}개</p></div><Button onClick={() => void inspect(library.id)} aria-label={`${library.artist_name} 운영 정보 보기`}>운영 정보 보기</Button></div><p className="mt-2 break-all text-xs text-muted-foreground">회원 {library.owner_id} · 최근 변경 {displayDate(library.updated_at)}</p></article>)}</div>
    {!loading && !libraries.length && <p className="mt-4 text-sm text-muted-foreground">검색 조건에 맞는 아카이브가 없습니다.</p>}
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted-foreground">전체 {index.total ?? libraries.length}건 · {page + 1}페이지</p><div className="flex gap-2"><Button disabled={loading || page === 0} onClick={() => void search(page - 1)}>이전</Button><Button disabled={loading || index.nextPage == null} onClick={() => index.nextPage != null && void search(index.nextPage)}>다음</Button></div></div>
    {selectedId && <Modal title={detail ? `${detail.library.data.artist.name} 운영 정보` : "아카이브 운영 정보"} onClose={() => { detailRequestId.current++; setSelectedId(null); setDetail(null); }}>{detail ? <LibraryOperationsDetail detail={detail} onRetry={retry} onResolve={resolveConflict} /> : <p role="status" className="text-sm">아카이브 운영 정보를 불러오고 있습니다.</p>}</Modal>}
  </section>;
}
