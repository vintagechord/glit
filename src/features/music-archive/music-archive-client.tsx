    ...
    <div className="space-y-5" data-testid="music-archive">
      ...
      {loading ? <p role="status" className="flex items-center justify-center gap-2 py-10"><LoaderCircle className="h-4 w-4 animate-spin" aria-hidden />내 음악을 불러오고 있습니다.</p> : loginRequired ? <Notice><Link className="font-bold underline" href="/login?next=%2Fmypage%2Fmusic">로그인하고 내 음악 관리 시작</Link></Notice> : index && <>
        {!artistId ? <>
          ...
          {index.nextPage != null && <Button disabled={busy} onClick={() => act(async () => { const result = await archiveRequest<ArchiveIndex>(`?page=${index.nextPage}`); setIndex((previous) => previous ? { ...result, libraries: [...previous.libraries, ...result.libraries.filter((item) => !previous.libraries.some((saved) => saved.id === item.id))] } : result); })}>아티스트 더 보기</Button>}
          ...
          {!release ? <>
            <section className="space-y-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-black">{data.artist.name}</h2><p className="mt-1 text-sm text-muted-foreground">앨범 {releases.length}개</p></div><div className="flex flex-wrap gap-2"><Button disabled={busy || !!activeJobs} onClick={() => setDialog({ type: "connect" })}><Search className="h-4 w-4" aria-hidden />앨범 찾아보기</Button>{importProviders.some((provider) => data.connections.some((connection) => connection.provider === provider.id)) && <Button disabled={busy || !!activeJobs} onClick={() => act(refreshMusic)}><RefreshCw className="h-4 w-4" aria-hidden />새 발매작 확인</Button>}</div></div><SyncStatus jobs={current.jobs} busy={busy} resumableProviders={importProviders.map((provider) => provider.id)} onResume={(jobId) => act(async () => { await archiveRequest("", { action: "resume", jobId }); await refreshDetail(artistId); })} /></section>
            ...
            <section className={`${panelClass} flex items-center gap-4`}><Cover small /><div className="min-w-0 flex-1"><h2 className="break-words text-xl font-black">{release.title}</h2><p className="mt-1 text-sm text-muted-foreground">{[release.releaseDate, `${releaseTracks.length}곡`].filter(Boolean).join(" · ")}</p></div><Button onClick={() => setDialog({ type: "release", release })}>수정</Button></section>
