...
export function SyncStatus({ jobs, busy, onResume }: { jobs: SyncJob[]; busy: boolean; resumableProviders?: string[]; onResume: (jobId: string) => void }) {
  ...
  return <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">아직 불러오지 못한 앨범이 있습니다.</p><Button disabled={busy} onClick={() => onResume(retry[0].id)}>다시 불러오기</Button></div>;
}
