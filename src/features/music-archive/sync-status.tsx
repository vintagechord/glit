...
export function SyncStatus({ jobs, busy, resumableProviders = [], onResume }: { jobs: SyncJob[]; busy: boolean; resumableProviders?: string[]; onResume: (jobId: string) => void }) {
  ...
  const latest = ordered.filter((job, index) => ordered.findIndex((item) => item.provider === job.provider && item.external_artist_id === job.external_artist_id) === index);
  ...
  function renderJob(job: SyncJob, historical = false) {
    ...
    return <div key={job.id} className="space-y-2 rounded-lg border border-border bg-background p-3 text-sm">
      ...
      {job.external_artist_id && latest.filter((item) => item.provider === job.provider).length > 1 && <p className="text-xs text-muted-foreground">아티스트 ID {job.external_artist_id}</p>}
