...
import { Badge, Button, Cover, External, Field, Notice, inputClass } from "./ui";

export function AdminAlbumEditor({ library, onSaved }: { library: ArchiveLibrary; onSaved: (library: ArchiveLibrary) => Promise<void> }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (busy) return; setBusy(true); setError(""); setMessage(""); setRefreshError("");
    try {
      ...
      const result = await archiveRequest<{ library: ArchiveLibrary }>("", { action: "admin-commands", libraryId: library.id, version: library.version, commands });
      setEditing(false); setMessage(targetId ? `${titles.length}곡을 추가했습니다.` : `앨범과 ${titles.length}곡을 회원 음악 관리에 추가했습니다.`);
      try { await onSaved(result.library); }
      catch (caught) { setRefreshError(caught instanceof Error ? caught.message : "최신 정보를 불러오지 못했습니다."); }
  ...
  return <section className="space-y-4" aria-label="회원 앨범 관리">
    ...
    {refreshError && <Notice><p>저장은 완료했지만 최신 정보를 다시 불러오지 못했습니다.</p><p className="mt-1 text-xs">{refreshError}</p><div className="mt-2 flex justify-end"><Button disabled={busy} onClick={() => void refreshSaved()}>저장 정보 다시 불러오기</Button></div></Notice>}
