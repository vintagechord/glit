...
async function updateClaim(job: ReviewJob, update: Record<string, unknown>) {
  const { data, error } = await createAdminClient().from("review_document_jobs").update({ ...update, updated_at: new Date().toISOString() }).eq("id", job.id).eq("lease_token", job.lease_token!).eq("version", job.version).gt("lease_until", new Date().toISOString()).select("id").maybeSingle();
...
async function persistOutput(job: ReviewJob, name: string, buffer: Buffer, type: string): Promise<ReviewJobOutput> {
  ...
  const { error } = await createAdminClient().from("review_document_artifacts").insert({ id, job_id: job.id, object_key: objectKey });
  jobDatabaseError(error);
  const digest = await putReviewObject(objectKey, buffer, type);
...
export async function processReviewJob(job: ReviewJob) {
  try {
    ...
    if (job.operation === "generate") {
      ...
      await updateClaim(job, { status: "validating" });
      // Generation verifies DOCX/ZIP structure. Rendering is reported separately, never implied.
      const outputs: ReviewJobOutput[] = [];
      for (const file of generated.files) {
        await updateClaim(job, { status: "validating" });
        outputs.push(await persistOutput(job, file.name, file.buffer, "application/vnd.openxmlformats-officedocument.wordprocessingml.document"));
      }
      const zip = await persistOutput(job, job.mode === "mv" ? "영등위_가사.zip" : "음반_심의자료.zip", generated.zip, "application/zip");
      await updateClaim(job, { outputs, zip_output: zip, result_version: job.version, template_version: generated.templateVersion,
        ...
        status: "completed", error_code: null, error_message: null, retryable: false, lease_token: null, lease_until: null });
    } else {
      ...
      if (!extracted) {
        if (job.input_kind === "urls") extracted = remapUrlSources(await extractUrls(inputSources.map((s) => s.url!), job.application_date), job);
        else {
          const files = [];
          for (const source of inputSources) {
            ...
            const buffer = await getReviewObject(source.objectKey, REVIEW_JOB_LIMITS.fileBytes);
        ...
        if (!reextract) await updateClaim(job, { extracted_data: extracted });
      }
      if (reextract) extracted = mergeRetryExtraction(job.snapshot_data!, extracted);
      await updateClaim(job, { status: "translating" });
      const translated = await translateReviewData(extracted);
      await updateClaim(job, { draft_data: translated, version: job.operation === "translate" || reextract ? job.version + 1 : job.version, status: "needs_review", error_code: null, error_message: null, retryable: false, lease_token: null, lease_until: null });
    }
  } catch (error) {
