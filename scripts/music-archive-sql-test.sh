...
archive_sql_image_id="$(docker images --quiet --filter "reference=$archive_sql_image" | head -n 1)"
cleanup_archive_sql() {
...
if [[ -z "$archive_sql_image_id" ]] || ! docker image inspect "$archive_sql_image_id" >/dev/null 2>&1; then
...
archive_sql_container_id="$(docker run --rm --detach --network none --env POSTGRES_HOST_AUTH_METHOD=trust "$archive_sql_image_id")"
